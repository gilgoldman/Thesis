...
Which then may be converted into Alcohol By Volume (ABV), the more common unit of alcohol measurement in micro-breweries, as follows \cite{Brewing_Science}:
...
For this work, the experiment equipment was a custom metal rig with a movable slide, the distance to which was measured using a ******* infra-red distance measurement sensor with an accuracy of ***********, and a screen shows the current measurement value.\\

\begin{figure}[H]
	\centering
	\caption{Remember - Butta hadn't told you the infra-red sensor's name yet!!! Bug him until he does!}
\end{figure}

The experiments procedure involved mounting the sensor on a the custom rig at a set point. A small object was placed on the custom rig's slide, which was moved to a random distance that was defined as zero, or the experiments starting point. From there the object was moved towards the sensor in 20 discrete steps of 1[mm] each, to provide 20 measurements. A python routine was implemented to measure the distance to the object 26 times, and return the average of these measurements as well as their standard deviation. The measurement results can be seen in Table 5.1:
...
It is easy to see that the sensor preforms rather well despite its low cost, with only slight variation in the measurement - a standard deviation of 0.6533.\\
...
The tests conducted in Chapter 5 proved sufficient to determine the accuracy of the HC-sr04 sensor and its appropriateness for use in this device.	The experiments conducted show an almost linear match 
...
The proposed device was assembled in its entirety for less than 13$\$$US, or 283.28 $CZK$. The total cost of device assembly was about 3.25$\%$ of its mechanically closest competitor, BrewGuard, a 400$\$$US device which failed due to its high price. As proven in Chapter 5, the device can reach an accuracy 
